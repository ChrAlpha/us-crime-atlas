...
export const LOS_ANGELES_META: ProviderMeta = {
  ...
  datasetName: 'Crime Data from 2020 to Present',
  endpoint: 'https://data.lacity.org/resource/2nrs-mtv8.json',
  sourceUrl: 'https://data.lacity.org/d/2nrs-mtv8',
  center: [-118.2437, 34.0522],
  zoom: 10.7,
  bounds: { west: -118.68, south: 33.7, east: -118.15, north: 34.34 },
  cadence: 'Updated regularly',
  delayNote: 'Records are preliminary and can be revised after publication.',
  precision: 'approximate',
  precisionNote: 'Public coordinates are approximate and may identify a block rather than a premises.',
...
const losAngelesAliases = aliases({
  ...
  description: ['premis_desc', 'weapon_desc'],
  location: ['location'],
  area: ['area_name'],
...
export function mapLosAngelesRow(row: Record<string, unknown>, fields = losAngelesFields) {
  return incidentFromSocrata(LOS_ANGELES_META, row, fields, {
    ...
    precisionLabel: 'Approximate block',
    combineTime: true,
  });
}
...
export const SEATTLE_META: ProviderMeta = {
  ...
  sourceUrl: 'https://data.seattle.gov/d/tazs-3rd5',
