...
const meta: ProviderMeta = {
  id: 'adaptive-test', city: 'Test City', state: 'TS', label: 'Test City, TS',
  agency: 'Test Police Department', datasetName: 'Adaptive incidents',
  endpoint: 'https://example.test/resource/abcd-1234.json', sourceUrl: 'https://example.test/d/abcd-1234',
  center: [-75, 40], zoom: 12, bounds: { west: -76, south: 39, east: -74, north: 41 },
  cadence: 'Daily', delayNote: 'Test delay', precision: 'approximate', precisionNote: 'Approximate',
  coverageNote: 'Test coverage', lastVerified: '2026-08-17',
};

const aliases = {
  id: ['incident_id', 'id'], date: ['occurred_at', 'date'], category: ['offense'],
  latitude: ['lat', 'latitude'], longitude: ['lon', 'longitude'], point: ['location'], optional: ['detail'],
} as const;

const resolved: ResolvedSocrataFields<TestKey> = {
  id: 'incident_id', date: 'occurred_at', category: 'offense', latitude: 'lat', longitude: 'lon',
  point: null, optional: null,
};
...
function mapRow(row: Record<string, unknown>, fields: ResolvedSocrataFields<TestKey>): Incident | null {
  ...
  return {
    id, providerId: meta.id, occurredAt, occurredAtEpochMs: Date.parse(occurredAt), localHour: 12,
    category: 'other', group: 'other', rawCategory: resolvedText(row, fields, 'category', 'Other'),
    description: 'Test', locationLabel: 'Test location', coordinates, severity: 1,
    isViolent: false, precision: 'approximate',
  };
}

const config: AdaptiveSocrataProviderConfig<TestKey> = {
  meta, aliases, requiredKeys: ['id', 'date', 'category'], dateKey: 'date',
  latitudeKey: 'latitude', longitudeKey: 'longitude', pointKey: 'point', mapRow,
};

const query: IncidentQuery = {
  center: [-75, 40], radiusMeters: 3000, windowDays: 30,
...
describe('adaptive Socrata provider boundary', () => {
  it('derives metadata URL and resolves the first available alias', () => {
    ...
    expect(resolveSocrataAliases(aliases, ['id', 'occurred_at', 'offense', 'latitude', 'longitude'])).toEqual({
      id: 'id', date: 'occurred_at', category: 'offense', latitude: 'latitude', longitude: 'longitude',
      point: null, optional: null,
    });
  ...
  it('builds bounded queries for separate coordinates and point geometry', () => {
    ...
    expect(url.searchParams.get('$where')).toContain('lat >=');
  ...
  it('resolves metadata once, fetches rows, filters invalid records, and reports provenance', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ columns: [
        { fieldName: 'incident_id' }, { fieldName: 'occurred_at' }, { fieldName: 'offense' },
        { fieldName: 'lat' }, { fieldName: 'lon' },
      ] }), { status: 200 }))
  ...
  it('surfaces metadata, semantic, HTTP, and payload failures', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({ columns: [{ fieldName: 'incident_id' }] }), { status: 200 })));
    await expect(createAdaptiveSocrataProvider(config).fetchIncidents(query)).rejects.toThrow('missing required fields');

    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({ columns: [
      { fieldName: 'incident_id' }, { fieldName: 'occurred_at' }, { fieldName: 'offense' },
    ] }), { status: 200 })));
  ...
  it('reads primitive fields and both Socrata point shapes safely', () => {
    ...
    expect(resolvedCoordinates(row, resolved, { latitudeKey: 'latitude', longitudeKey: 'longitude' })).toEqual([-75.5, 40.5]);

    const pointFields = { ...resolved, latitude: null, longitude: null, point: 'location' };
    expect(resolvedCoordinates({ location: { type: 'Point', coordinates: [-75, 40] } }, pointFields, { pointKey: 'point' })).toEqual([-75, 40]);
    expect(resolvedCoordinates({ location: { longitude: -74, latitude: 41 } }, pointFields, { pointKey: 'point' })).toEqual([-74, 41]);
    expect(resolvedCoordinates({ location: { coordinates: [0, 0] } }, pointFields, { pointKey: 'point' })).toBeNull();
  });
});
